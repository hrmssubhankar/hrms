  ...
  return (
    <div className="h-full flex flex-col">
      <div className="px-6 py-4 border-b border-gray-200 bg-white flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-gray-900">Contracting</h1>
          <p className="text-sm text-gray-500 mt-0.5">Employment contracts — draft, send, track signatures</p>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="w-80 border-r border-gray-200 flex flex-col bg-white">
          <div className="p-3 border-b border-gray-100 space-y-2">
            <input className="w-full border border-gray-200 rounded-lg px-3 py-1.5 text-sm" placeholder="Search…" value={search} onChange={e=>setSearch(e.target.value)}/>
            <select className="w-full border border-gray-200 rounded-lg px-2 py-1.5 text-xs" value={statusFilter} onChange={e=>setStatus(e.target.value)}>
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? <p className="p-4 text-sm text-gray-400">Loading…</p> : contracts.length===0 ? <p className="p-6 text-sm text-gray-400 text-center">No contracts</p> : contracts.map(c=>(
              <button key={c.id} onClick={()=>setSelected(c)}
                className={`w-full text-left px-4 py-3 border-b border-gray-100 hover:bg-gray-50 transition ${selected?.id===c.id?'bg-brand-50 border-l-2 border-l-brand-500':''}`}>
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-gray-900 truncate">{c.firstName} {c.lastName}</p>
                    <p className="text-xs text-gray-500 capitalize">{c.type.replace(/_/g,' ')}</p>
                  </div>
          ...
          <div className="flex-1 p-6 overflow-y-auto bg-white space-y-5">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-sm text-gray-500 capitalize">{selected.type.replace(/_/g,' ')} contract</p>
              </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" value={selected.status}
              ...
              <div>
                ...
                <input className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" defaultValue={selected.superFund??''} onBlur={e=>patch(selected.id,{superFund:e.target.value||null})}/>
              </div>
            </div>
      ...
      {showCreate && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-sm p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">New Contract</h3>
