  ...
  return (
    <div className="h-full flex flex-col">
      <div className="px-6 py-4 border-b border-gray-200 bg-white dark:bg-gray-900 dark:border-gray-700">
        <h1 className="text-xl font-semibold text-gray-900 dark:text-white">Employee Experience</h1>
        <p className="text-sm text-gray-500 mt-0.5 dark:text-gray-400">Tenure milestones and workforce longevity</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        {upcoming.length > 0 && (
          <div>
            <h2 className="text-sm font-semibold text-gray-700 mb-3 dark:text-gray-300">Upcoming Milestones (within 2 months)</h2>
              ...
              {upcoming.map(e => {
                ...
                return (
                  <div key={e.id} className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-gray-900 dark:text-white">{e.firstName} {e.lastName}</p>
                      <p className="text-xs text-amber-700">{label} milestone approaching!</p>
                      <p className="text-xs text-gray-500 dark:text-gray-400">Currently: {tenure(e.startDate)}</p>
                    </div>
                  </div>
                )
              })}
